...
from crop.model import Model
from crop.view import View
...
class Controller():
    ...
    def start_fixed_size_selection(self, center_coord: Tuple[int, int]):
        self.model.press_coord = center_coord
        self.model.move_coord = center_coord
        self.update_fixed_size_selection()

    def update_fixed_size_selection(self):
        selected_box = self.get_selected_box_for_fixed_size(
            self.model.press_coord, self.model.move_coord,
            self.model.args.fixed_size, self.model.current_image.size,
            self.model.canvas_image_dimensions)
    ...
    @staticmethod
    def calculate_canvas_image_dimensions(image_width: int, image_height: int,
    ...
    def get_selection_box_for_aspect_ratio(selection_box: Tuple[int, int, int, int],
                                           aspect_ratio: float, mouse_press_coord: Tuple[int, int],
    ...
    def get_selected_box(mouse_press_coord: Tuple[int, int],
        ...
        selection_bottom_right_y: int = max(
            mouse_press_coord[1], mouse_move_coord[1])
        selection_box: Tuple[int, int, int, 
    ...
    @staticmethod
    def get_real_box(selected_box: Tuple[int, int, int, int],
    ...
    @staticmethod
    def get_selected_box_for_fixed_size(press_coord: Tuple[int, int],
                                        move_coord: Tuple[int, int],
                                        fixed_size: Tuple[int, int],
                                        original_image_size: Tuple[int, int],
                                        displayed_image_size: Tuple[int, int]) -> Tuple[int, int, int, int]:
        center_x = (press_coord[0] + move_coord[0]) // 2
        center_y = (press_coord[1] + move_coord[1]) // 2

        half_width = fixed_size[0] * displayed_image_size[0] / (2 * original_image_size[0])
        half_height = fixed_size[1] * displayed_image_size[1] / (2 * original_image_size[1])

        x1 = max(0, center_x - half_width)
        y1 = max(0, center_y - half_height)
        x2 = min(displayed_image_size[0], center_x + half_width)
        y2 = min(displayed_image_size[1], center_y + half_height)
